...
public:
  ...
  void setGroup(bool bGroup) {if (m_bGroup==bGroup) return; m_bGroup=bGroup; emit groupChanged();}
  bool owner() const {return m_bOwner;}
  void setOwner(bool bOwner) {if (m_bOwner==bOwner) return; m_bOwner=bOwner; emit ownerChanged();}
  bool perms() const {return m_bPerms;}
  void setPerms(bool bPerms) {if (m_bPerms==bPerms) return; m_bPerms=bPerms; emit permsChanged();}
  bool links() const {return m_bLinks;}
  void setLinks(bool bLinks) {if (m_bLinks==bLinks) return; m_bLinks=bLinks; emit linksChanged();}
  bool update() const {return m_bUpdate;}
  void setUpdate(bool bUpdate) {if (m_bUpdate==bUpdate) return; m_bUpdate=bUpdate; emit updateChanged();}
  bool del() const {return m_bDel;}
  void setDel(bool bDel) {if (m_bDel==bDel) return; m_bDel=bDel; emit delChanged();}
  bool deleteExcluded() const {return m_bDeleteExcluded;}
  void setDeleteExcluded(bool bDeleteExcluded) {if (m_bDeleteExcluded==bDeleteExcluded) return; m_bDeleteExcluded=bDeleteExcluded; emit deleteExcludedChanged();}
  bool ignoreNonExisting() const {return m_bIgnoreNonExisting;}
  void setIgnoreNonExisting(bool bIgnoreNonExisting) {if (m_bIgnoreNonExisting==bIgnoreNonExisting) return; m_bIgnoreNonExisting=bIgnoreNonExisting; emit ignoreNonExistingChanged();}
  bool ignoreExisting() const {return m_bIgnoreExisting;}
  void setIgnoreExisting(bool bIgnoreExisting) {if (m_bIgnoreExisting==bIgnoreExisting) return; m_bIgnoreExisting=bIgnoreExisting; emit ignoreExistingChanged();}
  bool recursive() const {return m_bRecursive;}
  void setRecursive(bool bRecursive) {if (m_bRecursive==bRecursive) return; m_bRecursive=bRecursive; emit recursiveChanged();}
  bool maxSize() const {return m_bMaxSize;}
  void setMaxSize(bool bMaxSize) {if (m_bMaxSize==bMaxSize) return; m_bMaxSize=bMaxSize; emit maxSizeChanged();}
  quint32 maxSizeValue() const {return m_iMaxSizeValue;}
  void setMaxSizeValue(quint32 iMaxSizeValue) {if (m_iMaxSizeValue==iMaxSizeValue) return; m_iMaxSizeValue=iMaxSizeValue; emit maxSizeValueChanged();}
  quint32 unit() const {return m_iUnit;}
  void setUnit(quint32 iUnit) {if (m_iUnit==iUnit) return; m_iUnit=iUnit; emit unitChanged();}
  QString from() const {return m_sFrom;}
  void setFrom(const QString &sFrom) {if (m_sFrom==sFrom) return; m_sFrom=sFrom; emit fromChanged();}
  QString to() const {return m_sTo;}
  void setTo(const QString &sTo) {if (m_sTo==sTo) return; m_sTo=sTo; emit toChanged();}
  QString fromPort() const {return m_sFromPort;}
  void setFromPort(const QString &sFromPort) {if (m_sFromPort==sFromPort) return; m_sFromPort=sFromPort; emit fromPortChanged();}
  QString toPort() const {return m_sToPort;}
  void setToPort(const QString &sToPort) {if (m_sToPort==sToPort) return; m_sToPort=sToPort; emit toPortChanged();}

  QString confFileName() const {return m_sConfFileName;}
  void setConfFileName(const QString &sConfFileName) {if (m_sConfFileName==sConfFileName) return; m_sConfFileName=sConfFileName; emit confFileNameChanged();}
  QString errorMessage() const {return m_sErrorMessage;}
  void setErrorMessage(const QString &sErrorMessage) {if (m_sErrorMessage==sErrorMessage) return; m_sErrorMessage=sErrorMessage; emit errorMessageChanged();}
  bool isRunning() const {return m_bIsRunning;}
  void setIsRunning(bool bIsRunning) {if (m_bIsRunning==bIsRunning) return; m_bIsRunning=bIsRunning; emit isRunningChanged();}
  QString excIncHelp() const {return m_sExcIncHelp;}
  void setExcIncHelp(const QString &sExcIncHelp) {if (m_sExcIncHelp==sExcIncHelp) return; m_sExcIncHelp=sExcIncHelp; emit excIncHelpChanged();}
  QString help() const {return m_sHelp;}
  void setHelp(const QString &sHelp) {if (m_sHelp==sHelp) return; m_sHelp=sHelp; emit helpChanged();}
