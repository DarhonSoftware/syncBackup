...
CBackend::CBackend(QQmlApplicationEngine *pEngine) : QObject(pEngine)
{
  ...
  connect(&m_Process,QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),this,&CBackend::finishedProcess);
...
void CBackend::init()
{
  ...
  if (!sFileName.isEmpty())
    if (!loadConfiguration(sFileName)) sFileName.clear();
...
void CBackend::end()
{
  ...
  if (!sFileName.isEmpty())
    if (!saveConfiguration(sFileName)) sFileName.clear();
...
void CBackend::parseArguments(QStringList *plsArguments)
{
  ...
  if (group()) plsArguments->append("--group");
  if (owner()) plsArguments->append("--owner");
  if (perms()) plsArguments->append("--perms");
  if (links()) plsArguments->append("--links");
  if (update()) plsArguments->append("--update");
  if (del()) plsArguments->append("--delete");
  if (deleteExcluded()) plsArguments->append("--delete-excluded");
  if (ignoreNonExisting()) plsArguments->append("--ignore-non-existing");
  if (ignoreExisting()) plsArguments->append("--ignore-existing");
  if (recursive()) plsArguments->append("--recursive");
  if (maxSize()) plsArguments->append(QString("--max-size=%1%2").arg(maxSizeValue()).arg(m_lsUnits.at(unit())));

  if (fromPort()!="-1") plsArguments->append("--rsh=ssh -p "+fromPort());
  if (toPort()!="-1") plsArguments->append("--rsh=ssh -p "+toPort());
...
bool CBackend::run(bool bDryRun)
{
  ...
  if (bDryRun) lsArguments.append("--dry-run");
...
{
  if (s.isEmpty()) return tr("-no-config-file-");
...
bool CBackend::loadConfiguration(const QString &sFile)
{
  ...
  Stream>>bAux; setGroup(bAux);
  Stream>>bAux; setOwner(bAux);
  Stream>>bAux; setPerms(bAux);
  Stream>>bAux; setLinks(bAux);
  Stream>>bAux; setUpdate(bAux);
  Stream>>bAux; setDel(bAux);
  Stream>>bAux; setDeleteExcluded(bAux);
  Stream>>bAux; setIgnoreNonExisting(bAux);
  Stream>>bAux; setIgnoreExisting(bAux);
  Stream>>bAux; setRecursive(bAux);
  Stream>>bAux; setMaxSize(bAux);
  Stream>>uiAux; setMaxSizeValue(uiAux);
  Stream>>uiAux; setUnit(uiAux);
  Stream>>lsAux; stringListToModel(lsAux,&m_ModelExc);
  Stream>>lsAux; stringListToModel(lsAux,&m_ModelInc);
  Stream>>sAux; setFrom(sAux);
  Stream>>sAux; setTo(sAux);
  Stream>>sAux; setFromPort(sAux);
  Stream>>sAux; setToPort(sAux);
